...
int menu(state* game_state)
{
    ...
    while (!choosen) {
        ...
        switch (_ENDPOINT_P(msg.m_source)) {
            ...
            if (msg.m_notify.interrupts & keyboard_bit_mask)
            {
                if(analize_buffer())
                    return 1;
                if(process_packet())
                ...
                if(complete)
                {
                    ...
                    if(detect_colisions(cursor,play->x,play->y,play->width,play->height) && mouse_packets.lb)
                    {
                        choosen = 1;
                        (*game_state) = GAME;
                    }
                    else if(detect_colisions(cursor,exit->x,exit->y,exit->width,exit->height) && mouse_packets.lb)
                    {
                        choosen = 1;
                        (*game_state) = EXIT;
                    }
